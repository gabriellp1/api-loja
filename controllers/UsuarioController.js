...
class UsuarioController {
    ...
    index(req, res, next){
        Usuario.findById(req.payload.id).then(usuario =>{
            ...
            return res.json({ usuari: usuario.enviarAuthJSON() });
    ...
    show(req, res, next){
        Usuario.findById(req.params.id).populate({ patch: "loja" })
        .then(usuario =>{
            if(!usuario) return res.status(401).json({ errors: "Usuario não registrado"});
            return res.json({
                usuario: {
    ...
    store(req, res, next){
        ...
        if( !nome || !email || !password || !loja) return res.status(422).json({ errors: "Preencha os campos de cadastro." });
    ...
    update(req, res, next){
        ...
        Usuario.findById(req.payload.id).then((usuario) => {
            ...
            if(typeof nome !== "undefined") usuario.email = email;
            if(typeof nome !== "undefined") usuario.setSenha(password);
